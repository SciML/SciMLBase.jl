...
Jacobian-vector products when they are cheap.

Return `true` when the solver can consume `cons_jvp`, a callback applying the
constraint Jacobian to a vector, but has its own fallback (typically through
`cons_j`) when none is provided. Function-instantiation code should then only
provide `cons_jvp` when it does not materialize the full Jacobian: a
user-supplied `cons_jvp`, or one generated by an AD backend with a native
forward mode. Solvers that call `cons_jvp` unconditionally should declare
[`requiresconsjvp`](@ref) instead.
...
Trait declaring whether an optimization algorithm requires constraint
Jacobian-vector products.

Return `true` when constrained solver setup must obtain `cons_jvp`, even when
the only way to build it is through the full constraint Jacobian (for example
from a user-supplied `cons_j`, or with an AD backend that has no native forward
mode). A solver declaring it should also declare [`allowsconsjvp`](@ref).

The default is `false`.
"""
requiresconsjvp(opt) = false

"""
    allowsconsvjp(opt)

Trait declaring whether an optimization algorithm can use constraint
vector-Jacobian products when they are cheap.

Return `true` when the solver can consume `cons_vjp`, a callback applying the
adjoint action of the constraint Jacobian, but has its own fallback (typically
through `cons_j`) when none is provided. Function-instantiation code should then
only provide `cons_vjp` when it does not materialize the full Jacobian: a
user-supplied `cons_vjp`, or one generated by an AD backend with a native
reverse mode. Solvers that call `cons_vjp` unconditionally should declare
[`requiresconsvjp`](@ref) instead.
...
Return `true` when constrained solver setup must obtain `cons_vjp`, even when
the only way to build it is through the full constraint Jacobian (for example
from a user-supplied `cons_j`, or with an AD backend that has no native reverse
mode). A solver declaring it should also declare [`allowsconsvjp`](@ref).

The default is `false`.
"""
requiresconsvjp(opt) = false
