...
To implement this inteface, the solution must store a `SavedSubsystem` if it contains a
subset of all timeseries variables. It must forward `is_timeseries_parameter`,
`timeseries_parameter_index`, `with_updated_parameter_timeseries_values` and
`get_saveable_values` to `SavedSubsystemWithFallback(sol.saved_subsystem, symbolic_container(sol))`.
...
"""
struct SavedSubsystem{V, T, I, P, Q, C}
    """
...
function SavedSubsystem(indp, pobj, saved_idxs)
    ...
    for var in saved_idxs
        ...
        elseif (idx = timeseries_parameter_index(indp, var)) !== nothing
            ...
            val = parameter_values(pobj, parameter_index(indp, var))
            T = typeof(val)
    ...
    if save_all_tsparams
        ...
        return SavedSubsystem(
            state_map, nothing, identity_partitions, nothing, nothing, nothing)
    end

    if num_ts_params == 0
        return SavedSubsystem(state_map, nothing, (), nothing, nothing, nothing)
    end
    ...
    return SavedSubsystem(state_map, parammap, identitypartitions,
...
function SymbolicIndexingInterface.with_updated_parameter_timeseries_values(
    ...
    for (tsidx, val) in args
        ...
        for idx in ss.indexes_in_partition
            set_parameter!(ps, parameter_values(val, ss.timeseries_params_map[idx]), idx)
        end
    end
