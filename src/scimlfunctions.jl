...
function Base.showerror(io::IO, e::NonconformingFunctionsError)
    ...
    return printstyled(io, e.nonconforming; bold = true, color = :red)
end
...
function Base.showerror(io::IO, e::IntegrandMismatchFunctionError)
    ...
    return printstyled(io, e.integrand_passed; bold = true, color = :red)
end
...
struct ODEFunction{
    ...
    f::F
    g::G                    # from SDE: noise function
    mass_matrix::TMM
    analytic::Ta
    tgrad::Tt
    jac::TJ
    jac_u::TJU              # from DAE: partial Jacobian w.r.t. u
    jac_du::TJD             # from DAE: partial Jacobian w.r.t. du
    jvp::JVP
    vjp::VJP
    jac_prototype::JP
    sparsity::SP
    ...
    paramjac::TPJ
    vjp_p::VJP_P
    ggprime::GG             # from SDE: Milstein correction
    observed::O
    colorvec::TCV
    sys::SYS
    resid_prototype::RP     # from Nonlinear
    analytic_full::Bool     # from RODE
...
"""
const DDEFunction = ODEFunction

"""
SDEFunction is an alias for ODEFunction. SDEFunction uses the `g` and `ggprime`
fields for noise function and Milstein correction respectively.
"""
const SDEFunction = ODEFunction

"""
SDDEFunction is an alias for ODEFunction (same as SDEFunction).
"""
const SDDEFunction = ODEFunction

"""
RODEFunction is an alias for ODEFunction. RODEFunction uses the `analytic_full`
field.
"""
const RODEFunction = ODEFunction

"""
DAEFunction is an alias for ODEFunction. DAEFunction uses the `jac_u` and `jac_du`
fields for partial Jacobians.
"""
const DAEFunction = ODEFunction

"""
NonlinearFunction is an alias for ODEFunction. NonlinearFunction uses the
`resid_prototype` field.
"""
const NonlinearFunction = ODEFunction
...
struct SplitFunction{
        ...
        NLP <: Union{Nothing, ODENLStepData},
    } <: AbstractODEFunction{iip}
    f1::F1
    f2::F2
    mass_matrix::TMM
...
struct DynamicalODEFunction{
        iip, specialize, F1, F2, TMM, Ta, Tt, TJ, JVP, VJP, JP, SP, TW,
        TWt, TPJ,
        O, TCV, SYS, ID,
    } <: AbstractODEFunction{iip}
    f1::F1
...
"""
# DDEFunction struct removed — DDEFunction is now a const alias for ODEFunction
...
The fields of the ODEFunction type directly match the names of the inputs.
"""
# SDEFunction struct removed — SDEFunction is now a const alias for ODEFunction
...
struct SplitSDEFunction{
        iip, specialize, F1, F2, G, TMM, C, Ta, Tt, TJ, JVP, VJP, JP, SP,
        TW,
        TWt, TPJ,
        O, TCV, SYS, ID,
...
"""
# DynamicalSDEFunction is now an alias for SplitSDEFunction (they are identical structs).
const DynamicalSDEFunction = SplitSDEFunction
...
"""
# RODEFunction struct removed — RODEFunction is now a const alias for ODEFunction
...
numerically-defined functions.
"""
# DAEFunction struct removed — DAEFunction is now a const alias for ODEFunction
...
The fields of the DDEFunction type directly match the names of the inputs.
"""
# SDDEFunction struct removed — SDDEFunction is now a const alias for ODEFunction
...
"""
# NonlinearFunction struct removed — NonlinearFunction is now a const alias for ODEFunction
...
"""
struct ODEInputFunction{
        iip, specialize, F, TMM, Ta, Tt, TJ, CTJ, JVP, VJP,
        JP, CJP, SP, TW, TWt, WP, TPJ, O, TCV,
        SYS, ID,
    } <: AbstractODEInputFunction{iip}
    f::F
    mass_matrix::TMM
    analytic::Ta
    tgrad::Tt
...
# NonlinearFunction callable is handled by ODEFunction since it's now a const alias
...
# DAEFunction and DDEFunction callables handled by ODEFunction since they are const aliases
...
# SDEFunction and SDDEFunction callables handled by ODEFunction since they are const aliases
...
# RODEFunction callable handled by ODEFunction since it's a const alias
...
function ODEFunction{iip, specialize}(
    ...
    return if specialize === NoSpecialize
        ODEFunction{
            ...
            Any, Any, Any,
            typeof(_colorvec),
            typeof(sys), Any, Union{Nothing, OverrideInitData}, Union{Nothing, ODENLStepData},
        }(
            _f, _g, mass_matrix, analytic, tgrad, jac, jac_u, jac_du,
            jvp, vjp, jac_prototype, sparsity, Wfact,
            Wfact_t, W_prototype, paramjac, vjp_p, ggprime,
            observed, _colorvec, sys, resid_prototype, analytic_full,
    ...
    elseif specialize === false
        ODEFunction{
            ...
            typeof(paramjac),
            typeof(vjp_p), typeof(ggprime),
            typeof(observed),
            typeof(_colorvec),
            typeof(sys), typeof(resid_prototype), typeof(initdata), typeof(nlstep_data),
        }(
            _f, _g, mass_matrix,
            analytic, tgrad, jac, jac_u, jac_du,
            jvp, vjp, jac_prototype, sparsity, Wfact,
            Wfact_t, W_prototype, paramjac, vjp_p, ggprime,
            observed, _colorvec, sys, resid_prototype, analytic_full,
            initdata, nlstep_data
        ...
        ODEFunction{
            ...
            _f, _g, mass_matrix, analytic, tgrad,
            jac, jac_u, jac_du, jvp, vjp, jac_prototype, sparsity, Wfact,
            Wfact_t, W_prototype, paramjac, vjp_p, ggprime,
            observed, _colorvec, sys, resid_prototype, analytic_full,
            initdata, nlstep_data
        )
...
function unwrapped_f(f::ODEFunction, newf = unwrapped_f(f.f))
    ...
    return if specialization(f) === NoSpecialize
        ODEFunction{
            ...
            typeof(f.sys), Any, Union{Nothing, OverrideInitData}, Union{Nothing, ODENLStepData},
        }(
            newf, f.g, f.mass_matrix, f.analytic, f.tgrad, f.jac, f.jac_u, f.jac_du,
            f.jvp, f.vjp, f.jac_prototype, f.sparsity, f.Wfact,
            f.Wfact_t, f.W_prototype, f.paramjac, f.vjp_p, f.ggprime,
            f.observed, f.colorvec, f.sys, f.resid_prototype, f.analytic_full,
            f.initialization_data, f.nlstep_data
        )
    elseif specialization(f) === AutoSpecialize
        ODEFunction{
            ...
            typeof(f.jvp), typeof(f.vjp), typeof(f.jac_prototype),
            typeof(f.sparsity), typeof(f.Wfact), typeof(f.Wfact_t), typeof(f.W_prototype),
            typeof(f.paramjac),
            typeof(f.vjp_p), typeof(f.ggprime),
            typeof(f.observed), typeof(f.colorvec),
            typeof(f.sys), typeof(f.resid_prototype),
            typeof(f.initialization_data), typeof(f.nlstep_data),
        }(
            newf, f.g, f.mass_matrix, f.analytic, f.tgrad, f.jac, f.jac_u, f.jac_du,
            f.jvp, f.vjp, f.jac_prototype, f.sparsity, f.Wfact,
            f.Wfact_t, f.W_prototype, f.paramjac, f.vjp_p, f.ggprime,
...
# ODEFunction and NonlinearFunction are now the same type (const alias),
# so conversion functions are no longer needed.

# unwrapped_f for NonlinearFunction is handled by unwrapped_f(f::ODEFunction, ...) since they are the same type.
...
function SplitFunction{iip, specialize}(
        f1, f2;
    ...
    return if specialize === NoSpecialize
        SplitFunction{
            iip, specialize, Any, Any, Any, Any, Any, Any, Any, Any, Any,
...
@add_kwonly function DynamicalODEFunction{iip}(
        f1, f2, mass_matrix, analytic, tgrad, jac,
        jvp, vjp, jac_prototype, sparsity, Wfact,
        Wfact_t, paramjac,
        observed, colorvec, sys, initialization_data = nothing
    ) where {iip}
...
function DynamicalODEFunction{iip, specialize}(
        f1, f2;
        mass_matrix = __has_mass_matrix(f1) ?
            f1.mass_matrix : I,
        analytic = __has_analytic(f1) ? f1.analytic :
    ...
    return if specialize === NoSpecialize
        DynamicalODEFunction{
            iip, specialize, Any, Any, Any, Any, Any, Any, Any,
            Any, Any, Any, Any, Any,
            Any, Any, Any, Any, Any,
        }(
            f1, f2, mass_matrix,
...
function DiscreteFunction{iip, specialize}(
        f;
    ...
    return if specialize === NoSpecialize
        ...
        DiscreteFunction{
            ...
            typeof(observed), typeof(sys), typeof(initialization_data),
        }(
            _f,
...
function unwrapped_f(f::DiscreteFunction, newf = unwrapped_f(f.f))
    ...
    return if specialize === NoSpecialize
        DiscreteFunction{
            isinplace(f), specialize, Any, Any,
            Any, Any, Any,
        }(newf, f.analytic, f.observed, f.sys)
    else
        DiscreteFunction{
            isinplace(f), specialize, typeof(newf), typeof(f.analytic),
...
function ImplicitDiscreteFunction{iip, specialize}(
        f;
...
function unwrapped_f(f::ImplicitDiscreteFunction, newf = unwrapped_f(f.f))
    ...
    return if specialize === NoSpecialize
        ImplicitDiscreteFunction{
            isinplace(f, 5), specialize, Any, Any, Any,
            Any, Any, Any,
        }(
        ...
        ImplicitDiscreteFunction{
            isinplace(f, 5), specialize, typeof(newf),
            typeof(f.analytic),
...
function SDEFunction{iip, specialize}(
        f, g;
        mass_matrix = __has_mass_matrix(f) ? f.mass_matrix :
            I,
        analytic = __has_analytic(f) ? f.analytic : nothing,
    ...
    # Check g conformance if g is not nothing
    if g !== nothing
        giip = isinplace(g, 4, "g", iip)
        if giip != iip
            throw(NonconformingFunctionsError(["g"]))
        end
    end

    return ODEFunction{iip, specialize}(
        f;
        g = g,
        ...
        nlstep_data = nlstep_data
    )
end

# unwrapped_f for SDEFunction is handled by unwrapped_f(f::ODEFunction, ...) since they are the same type.
...
@add_kwonly function SplitSDEFunction(
    ...
    SplitFunction{
        isinplace(f2), typeof(f1), typeof(f2), typeof(g), typeof(mass_matrix),
        typeof(_func_cache), typeof(analytic), typeof(tgrad), typeof(jac), typeof(jvp),
        typeof(vjp),
        typeof(Wfact), typeof(Wfact_t), typeof(paramjac), typeof(observed),
        typeof(colorvec),
...
function SplitSDEFunction{iip, specialize}(
        f1, f2, g;
        mass_matrix = __has_mass_matrix(f1) ?
            f1.mass_matrix :
            I,
    ...
    return if specialize === NoSpecialize
        SplitSDEFunction{
            iip, specialize, Any, Any, Any, Any, Any, Any,
            Any, Any, Any, Any, Any, Any, Any, Any, Any,
            Any, Any, Any, Any,
        }(
            f1, f2, g, mass_matrix, _func_cache,
            analytic,
        ...
        SplitSDEFunction{
            iip, specialize, typeof(f1), typeof(f2), typeof(g),
...
# DynamicalSDEFunction is now an alias for SplitSDEFunction.
# DynamicalSDEFunction constructors are the same as SplitSDEFunction's.

# RODEFunction is now a const alias for ODEFunction.
# RODEFunction{iip}(f; ...) and RODEFunction(f; ...) are handled by ODEFunction constructors.
# Note: RODEProblem constructors explicitly pass isinplace(f, 5) for correct RODE arg count.

# DAEFunction is now a const alias for ODEFunction.
# DAEFunction{iip}(f; ...) and DAEFunction(f; ...) are handled by ODEFunction constructors.
# Note: DAEProblem constructors explicitly pass isinplace(f, 5) for correct DAE arg count.

# DDEFunction is now a const alias for ODEFunction.
# DDEFunction{iip}(f; ...) and DDEFunction(f; ...) are handled by ODEFunction constructors.
# Note: DDEProblem constructors explicitly pass isinplace(f, 5) for correct DDE arg count.
...
function DynamicalDDEFunction{iip, specialize}(
        f1, f2;
        mass_matrix = __has_mass_matrix(f1) ?
            f1.mass_matrix : I,
        analytic = __has_analytic(f1) ? f1.analytic :
...
# SDDEFunction is now a const alias for ODEFunction (same as SDEFunction).
# SDDEFunction{iip, specialize}(f, g; ...) is the same method as SDEFunction{iip, specialize}(f, g; ...).
# SDDEProblem constructors handle the correct isinplace(f, 5) detection.

# NonlinearFunction is now a const alias for ODEFunction.
# NonlinearFunction{iip}(f; ...) and NonlinearFunction(f; ...) are handled by ODEFunction constructors.
# Note: NonlinearProblem constructors explicitly pass isinplace(f, 3) for correct nonlinear arg count.

function HomotopyNonlinearFunction{iip, specialize}(
        f;
    ...
    return if specialize === NoSpecialize
        ...
        HomotopyNonlinearFunction{
            iip, specialize, typeof(f), typeof(polynomialize),
...
function IntervalNonlinearFunction{iip, specialize}(
        f;
        analytic = __has_analytic(f) ?
            f.analytic :
            nothing,
...
function OptimizationFunction{iip}(
...
function MultiObjectiveOptimizationFunction(f, args...; kwargs...)
    isinplace(f, 3)
    return MultiObjectiveOptimizationFunction{true}(f, args...; kwargs...)
end

# Constructor with keyword arguments
function MultiObjectiveOptimizationFunction{iip}(
        f, adtype::AbstractADType = NoAD();
...
function BVPFunction{iip, specialize, twopoint}(
        f, bc;
        cost = __has_cost(f) ? f.cost : nothing,
        equality = __has_equality(f) ? f.equality : nothing,
        inequality = __has_inequality(f) ? f.inequality : nothing,
    ...
    nonconforming = (
        bciip, jaciip, tgradiip, jvpiip, vjpiip, Wfactiip, Wfact_tiip,
        paramjaciip, costiip, equalityiip, inequalityiip,
    ) .!= iip
    bc_nonconforming = bcjaciip .!= bciip
    if any(nonconforming)
        nonconforming = findall(nonconforming)
        functions = [
            "bc", "jac", "tgrad", "jvp", "vjp", "Wfact", "Wfact_t",
            "paramjac", "cost", "equality", "inequality",
        ][nonconforming]
    ...
    return if specialize === NoSpecialize
        BVPFunction{
            iip, specialize, twopoint, Any, Any, Any, Any, Any, Any, Any, Any, Any,
        ...
        BVPFunction{
            iip, specialize, twopoint, typeof(_f), typeof(bc), typeof(cost),
...
function DynamicalBVPFunction{iip, specialize, twopoint}(
        f, bc;
        cost = __has_cost(f) ? f.cost : nothing,
        equality = __has_equality(f) ? f.equality : nothing,
        inequality = __has_inequality(f) ? f.inequality : nothing,
    ...
    if any(nonconforming)
        nonconforming = findall(nonconforming)
        functions = [
            "bc", "jac", "bcjac", "tgrad", "jvp", "vjp", "Wfact", "Wfact_t",
            "paramjac", "cost", "equality", "inequality",
        ][nonconforming]
        throw(NonconformingFunctionsError(functions))
    end
...
function DynamicalBVPFunction{iip}(
        f, bc; twopoint::Union{Val, Bool} = Val(false),
        kwargs...
    ) where {iip}
...
function IntegralFunction{iip, specialize}(f, integrand_prototype) where {iip, specialize}
    _f = prepare_function(f)
    return IntegralFunction{iip, specialize, typeof(_f), typeof(integrand_prototype)}(
        _f,
        integrand_prototype
    )
end
...
function IntegralFunction(f)
    ...
    return IntegralFunction{false}(f, nothing)
end
function IntegralFunction(f, integrand_prototype)
    calculated_iip = isinplace(f, 3, "integral", true)
    return IntegralFunction{calculated_iip}(f, integrand_prototype)
end

function BatchIntegralFunction{iip, specialize}(
        f, integrand_prototype;
        max_batch::Integer = typemax(Int)
    ) where {iip, specialize}
    _f = prepare_function(f)
    return BatchIntegralFunction{
        iip,
        specialize,
        typeof(_f),
...
function BatchIntegralFunction{iip}(
        f,
        integrand_prototype;
        kwargs...
    ) where {iip}
...
function BatchIntegralFunction(f; kwargs...)
    ...
    return BatchIntegralFunction{false}(f, nothing; kwargs...)
end
function BatchIntegralFunction(f, integrand_prototype; kwargs...)
    calculated_iip = isinplace(f, 3, "batchintegral", true)
    return BatchIntegralFunction{calculated_iip}(f, integrand_prototype; kwargs...)
end

function ODEInputFunction{iip, specialize}(
        f;
        mass_matrix = __has_mass_matrix(f) ? f.mass_matrix :
            I,
        analytic = __has_analytic(f) ? f.analytic : nothing,
    ...
    if controljac === nothing && isa(controljac_prototype, AbstractSciMLOperator)
        if iip_bc
            ...
            controljac = (
                x, u, p, t,
            ) -> update_coefficients(deepcopy(controljac_prototype), u, p, t)
        end
    end
...
function __has_initializeprob(f)
    return has_initialization_data(f) && hasfield(typeof(f.initialization_data), :initializeprob)
end
function __has_update_initializeprob!(f)
    return has_initialization_data(f) &&
...
function has_update_initializeprob!(f::AbstractSciMLFunction)
    return __has_update_initializeprob!(f) &&
        f.initialization_data.update_initializeprob! !== nothing
end
function has_initializeprobmap(f::AbstractSciMLFunction)
    return __has_initializeprobmap(f) && f.initialization_data.initializeprobmap !== nothing
end
...
function has_jac(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_jac(f) && f.jac !== nothing) || has_jac(f.f1)
end
function has_jvp(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_jvp(f) && f.jvp !== nothing) || has_jvp(f.f1)
end
function has_vjp(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_vjp(f) && f.vjp !== nothing) || has_vjp(f.f1)
end
function has_tgrad(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_tgrad(f) && f.tgrad !== nothing) || has_tgrad(f.f1)
end
function has_Wfact(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_Wfact(f) && f.Wfact !== nothing) || has_Wfact(f.f1)
end
function has_Wfact_t(f::Union{SplitFunction, SplitSDEFunction})
    return (__has_Wfact_t(f) && f.Wfact_t !== nothing) || has_Wfact_t(f.f1)
end
...
function is_split_function(::Type{T}) where {
        T <: Union{
            SplitFunction, SplitSDEFunction, DynamicalODEFunction,
            DynamicalDDEFunction, HomotopyNonlinearFunction,
        },
    }
    return true
end
...
# DynamicalSDEFunction is now SplitSDEFunction, so no separate dispatch needed
...
function IncrementingODEFunction{iip, specialize}(f) where {iip, specialize}
    _f = prepare_function(f)
    return IncrementingODEFunction{iip, specialize, typeof(_f)}(_f)
end
...
for S in [
        :ODEFunction
        :DiscreteFunction
        # DAEFunction, DDEFunction, SDEFunction, RODEFunction, SDDEFunction, NonlinearFunction
        # are const aliases for ODEFunction, so they are already covered above
        :IntervalNonlinearFunction
...
function SymbolicIndexingInterface.symbolic_container(fn::AbstractSciMLFunction)
    return has_sys(fn) ? fn.sys : EMPTY_SYMBOLCACHE
end

function SymbolicIndexingInterface.is_observed(fn::AbstractSciMLFunction, sym)
    return has_sys(fn) ? is_observed(fn.sys, sym) : has_observed(fn)
end
...
function SymbolicIndexingInterface.is_observed(fn::HomotopyNonlinearFunction, sym)
    return is_observed(symbolic_container(fn), sym)
end
function SymbolicIndexingInterface.observed(fn::HomotopyNonlinearFunction, sym)
    return SymbolicIndexingInterface.observed(symbolic_container(fn), sym)
end
