...
function promote_dual(
        ::Type{T},
        ::Type{T2}
    ) where {T <: ForwardDiff.Dual, T2 <: ForwardDiff.Dual}
    return T
end
...
function promote_dual(
        ...
        T3, T4, V, V2 <: ForwardDiff.Dual, N, N2,
...
function promote_dual(
        ...
        T3, T4, V <: ForwardDiff.Dual, V2, N, N2,
        T <: ForwardDiff.Dual{T3, V, N},
        T2 <: ForwardDiff.Dual{T4, V2, N2},
    }
    return T
...
function promote_dual(
    ...
    return ForwardDiff.Dual{T3, promote_dual(V, V2), N}
end
...
function (dec::DualEltypeChecker)(::Val{Y}) where {Y}
    ...
    return anyeltypedual(getfield(dec.x, Y), Val{getval(dec.counter)})
end
...
@generated function anyeltypedual(x, ::Type{Val{counter}}) where {counter}
    ...
    elseif counter < DUALCHECK_RECURSION_MAX
        T = diffeqmapreduce(
            x -> anyeltypedual(x, Val{counter + 1}), promote_dual,
            x.parameters
        )
        if T === Any || isconcretetype(T)
            ...
            :(
                diffeqmapreduce(
                    DualEltypeChecker($x, $counter + 1), promote_dual,
...
function Base.showerror(io::IO, e::ForwardDiffAutomaticDetectionFailure)
    return print(io, FORWARDDIFF_AUTODETECTION_FAILURE_MESSAGE)
end
...
function anyeltypedual(
        ::Type{<:AbstractTimeseriesSolution{T, N}},
        ::Type{Val{counter}} = Val{0}
    ) where {T, N, counter}
    return anyeltypedual(T)
end
...
function anyeltypedual(
        ::Type{T}, ::Type{Val{counter}} = Val{0}
    ) where {counter} where {T <: NTuple}
    return __anyeltypedual_ntuple(T)
end

# Any in this context just means not Dual
function anyeltypedual(
        x::SciMLBase.NullParameters, ::Type{Val{counter}} = Val{0}
    ) where {counter}
    return Any
end
...
function anyeltypedual(
    ...
    return anyeltypedual((prob.u0, prob.p, prob.tspan))
end
...
function anyeltypedual(
    ...
    return anyeltypedual((prob.u0, prob.p))
end

function anyeltypedual(x::Number, ::Type{Val{counter}} = Val{0}) where {counter}
    return anyeltypedual(typeof(x))
end
function anyeltypedual(
        x::Union{String, Symbol}, ::Type{Val{counter}} = Val{0}
    ) where {counter}
    return typeof(x)
end
function anyeltypedual(
        x::Union{AbstractArray{T}, Set{T}},
        ::Type{Val{counter}} = Val{0}
    ) where {counter} where {
        T <:
        Union{
...
function anyeltypedual(
    ...
    return anyeltypedual(eltype(x))
end
function anyeltypedual(
        x::Union{AbstractArray{T}, Set{T}},
        ::Type{Val{counter}} = Val{0}
    ) where {counter} where {N, T <: NTuple{N, <:Number}}
    return anyeltypedual(eltype(x))
end
...
function anyeltypedual(x::AbstractArray, ::Type{Val{counter}} = Val{0}) where {counter}
    return if isconcretetype(eltype(x))
...
function anyeltypedual(x::AbstractDict, ::Type{Val{counter}} = Val{0}) where {counter}
    return isempty(x) ? eltype(values(x)) : mapreduce(anyeltypedual, promote_dual, values(x))
end
function anyeltypedual(x::NamedTuple, ::Type{Val{counter}} = Val{0}) where {counter}
    return anyeltypedual(values(x))
end

function anyeltypedual(
        f::SciMLBase.AbstractSciMLFunction, ::Type{Val{counter}}
    ) where {counter}
    return Any
end
...
function anyeltypedual(
        x::Union{ForwardDiff.AbstractConfig, Module},
        ::Type{Val{counter}} = Val{0}
    ) where {counter}
    return Any
end
...
function anyeltypedual(
        x::ForwardDiff.DiffResults.DiffResult,
        ::Type{Val{counter}} = Val{0}
    ) where {counter}
    return Any
end
...
function anyeltypedual(
        ::Type{T},
        ::Type{Val{counter}} = Val{0}
    ) where {counter} where {T <: ForwardDiff.Dual}
    return T
end
